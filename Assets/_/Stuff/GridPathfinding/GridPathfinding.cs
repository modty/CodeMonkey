...
namespace GridPathfindingSystem {
    ...
    public class GridPathfinding {
        ...
        public GridPathfinding(Vector3 worldLowerLeft, Vector3 worldUpperRight, float nodeSize, Axis axis) {
            instance = this;
            ...
            float worldHeight;
            if (axis == Axis.XY) {
                worldHeight = worldUpperRight.y - worldLowerLeft.y;
            } else {
                worldHeight = worldUpperRight.z - worldLowerLeft.z;
            }
        ...
        public GridPathfinding(int mapWidth, int mapHeight, float nodeSize, Vector3 worldOrigin, Axis axis) {//, Texture2D map) {
            this.nodeSize = nodeSize;
        ...
        public void RaycastWalkable() {
            for (int i = 0; i < widthMax; i++) {
                for (int j = 0; j < heightMax; j++) {
                    ...
                    RaycastHit2D raycastHit2D = Physics2D.Raycast(nodeWorldPosition, Vector2.zero, 0f);
                    if (raycastHit2D.collider != null) {
                        mapNodes[i][j].SetWalkable(false);
                    } else {
                        //if (Physics.Raycast(nodeWorldPosition, Vector3.zero, out RaycastHit raycastHit)) {
                        if (Physics.OverlapSphere(nodeWorldPosition, .1f).Length > 0) {
                            // Hit something
                            mapNodes[i][j].SetWalkable(false);
                        }
                    }
            ...
            for (int i = 0; i < newWidth; i++) {
                ...
                for (int j = 0; j < newHeight; j++) {
                    if (i < mapNodes.Length && j < mapNodes[0].Length) {
                        ...
                        newMapNodes[i][j] = new PathNode(i, j, axis);
        ...
        public void Initialize(int mapWidth, int mapHeight) {
            ...
            for (int x = 0; x < mapNodes.Length; x++) {
                for (int y = 0; y < mapNodes[x].Length; y++) {
                    mapNodes[x][y] = new PathNode(x, y, axis);
                }
            }
            UpdateNodeConnections();
        }
        ...
        public void PrintMap(Action<Vector3, Vector3, Color> createSprite) {
            PrintMap(
                (int x, int y) => {
                    ...
                    if (axis == Axis.XY) {
                        worldPosition = worldOrigin + new Vector3(x * nodeSize, y * nodeSize);
                    } else {
                        worldPosition = worldOrigin + new Vector3(x * nodeSize, 0, y * nodeSize);
                    }
                    createSprite(worldPosition, new Vector3(2, 2), Color.green);
                ...
                (int x, int y) => {
                    ...
                    createSprite(worldPosition, new Vector3(2, 2), Color.red);
        ...
        public List<PathNode> GetPath(Vector3 start, Vector3 end) {
            ...
            MapPos endMapPos;
            if (axis == Axis.XY) {
                startMapPos = GetClosestValidPos(Mathf.RoundToInt(start.x), Mathf.RoundToInt(start.y));
                endMapPos = GetClosestValidPos(Mathf.RoundToInt(end.x), Mathf.RoundToInt(end.y));
            } else {
                startMapPos = GetClosestValidPos(Mathf.RoundToInt(start.x), Mathf.RoundToInt(start.z));
                endMapPos = GetClosestValidPos(Mathf.RoundToInt(end.x), Mathf.RoundToInt(end.z));
            }
