...
public class GridCombatSystem : MonoBehaviour {
    ...
    private void UpdateValidMovePositions() {
        GridXZ<GridObject> grid = GameHandler_GridCombatSystem.Instance.GetGrid();
        GridPathfinding gridPathfinding = GameHandler_GridCombatSystem.Instance.gridPathfinding;

        // Get Unit Grid Position X, Y
        grid.GetXZ(unitGridCombat.GetPosition(), out int unitX, out int unitY);
        ...
        switch (state) {
            case State.Normal:
                if (Input.GetMouseButtonDown(0)) {
                    GridXZ<GridObject> grid = GameHandler_GridCombatSystem.Instance.GetGrid();
                    GridObject gridObject = grid.GetGridObject(GameHandler_GridCombatSystem.Instance.GetMouseWorldPosition());
                    ...
                    if (gridObject.GetIsValidMovePosition()) {
                        ...
                        if (canMoveThisTurn) {
                            ...
                            unitGridCombat.MoveTo(GameHandler_GridCombatSystem.Instance.GetMouseWorldPosition(), () => {
                                state = State.Normal;
                                UpdateValidMovePositions();
                                TestTurnOver();
                            });
