...
public class MovementTilemapVisual : MonoBehaviour {
    ...
    public void SetGrid(MovementTilemap tilemap, GridXZ<MovementTilemap.TilemapObject> grid) {
        this.grid = grid;
        UpdateTileMapVisual();
    ...
    private void Grid_OnGridValueChanged(object sender, GridXZ<MovementTilemap.TilemapObject>.OnGridObjectChangedEventArgs e) {
        updateMesh = true;
    }
    ...
    private void UpdateTileMapVisual() {
        ...
        for (int x = 0; x < grid.GetWidth(); x++) {
            for (int y = 0; y < grid.GetHeight(); y++) {
                ...
                MeshUtils.AddToMeshArraysXZ(vertices, uv, triangles, index, grid.GetWorldPosition(x, y) + quadSize * .5f, 0f, quadSize, gridUV00, gridUV11);
            }
        }
